...
MPCController::MPCController() : nh_(), received_applanix_state_(false),received_trajectory_(false), 
        ...
        run_controller_(false), sim_flag(0) {
p_hertz = 20;
//p_horizon = 20;
...
if (sim_flag == 1){
  //steering_pub_ = nh_.advertise<dbw_mkz_msgs::SteeringCmd>("path_follower/steering_cmd_sim",1);
  //steering_pub_ = nh_.advertise<dbw_mkz_msgs::SteeringCmd>("path_follower/steering_cmd",1);
  twist_pub_ = nh_.advertise<geometry_msgs::TwistStamped>("twist_raw_sim", 1);
  s_next_pub_ = nh_.advertise<path_follower_msgs::ApplanixPose>("s_next_sim", 30);
  s_close_pub_ = nh_.advertise<path_follower_msgs::ApplanixPose>("s_close_sim",30);
  des_traj_pub_ =nh_.advertise<path_follower_msgs::traj_plan>("des_traj_sim",30);
  applanix_pub_ = nh_.advertise<path_follower_msgs::ApplanixPose>("Applanix_sim",30);
  Uout_pub_ = nh_.advertise<path_follower_msgs::Uout>("Uout_sim",50);
  time_pub_ = nh_.advertise<path_follower_msgs::MpcTime>("MpcTime_sim",30);
  steering_current_pub_ = nh_.advertise<path_follower_msgs::SteeringCurrent>("path_follower/steering_current_sim",1);
}
else {
  //steering_pub_ = nh_.advertise<path_follower::SteeringCmd>("path_follower/steering_cmd",1);
  steering_pub_ = nh_.advertise<dbw_mkz_msgs::SteeringCmd>("path_follower/steering_cmd",1);
  twist_pub_ = nh_.advertise<geometry_msgs::TwistStamped>("twist_raw", 1);
  s_next_pub_ = nh_.advertise<path_follower_msgs::ApplanixPose>("s_next", 30);
  s_close_pub_ = nh_.advertise<path_follower_msgs::ApplanixPose>("s_close",30);
  des_traj_pub_ =nh_.advertise<path_follower_msgs::traj_plan>("des_traj",30);
  
  applanix_pub_ = nh_.advertise<path_follower_msgs::ApplanixPose>("Applanix",30);
  Uout_pub_ = nh_.advertise<path_follower_msgs::Uout>("Uout",50);
  time_pub_ = nh_.advertise<path_follower_msgs::MpcTime>("MpcTime",30);
  steering_current_pub_ = nh_.advertise<path_follower_msgs::SteeringCurrent>("path_follower/steering_current",1);

  // visualization of dest trajectory
  vis_pub_ = nh_.advertise<visualization_msgs::MarkerArray>("target_waypoints_mark", 15, true);
...
void MPCController::changeParams() {
  ...
  p_Q2.block(0,0,6,6) = p_Q;
  p_Q2(6,6) = p_q_theta_ddot;
...
void MPCController::currentstateHandler(const path_follower_msgs::state_Dynamic current_state){
  applanix_.vel_north  = current_state.vy;
  applanix_.vel_east   = current_state.vx;
...
visualization_msgs::Marker MPCController::displayWayPoint(geometry_msgs::Point target) {
...
void MPCController::publishTargetWaypointsMarkers(){
  visualization_msgs::MarkerArray marker_array;

  //insert global marker
  marker_array.markers.insert(marker_array.markers.end(), target_waypoints_marker_array.markers.begin(),
                              target_waypoints_marker_array.markers.end());
...
void MPCController::controlLoop() {
 ...
 /* if (traj_.header.stamp.toSec() < applanix_.timestamp - 10.0 ) {
  	cout<< traj_.header.stamp.toSec();
  	cout<< applanix_.timestamp;
    controls_.setZero();
    ctl_err_.setZero();
    ctl_err_vel_.setZero();
    ctl_err_int_.setZero();
...
{  
  ...
  state_(3) = applanix_.vel_east;
  state_(4) = applanix_.vel_north + state_(5) * imu_to_cg;
}

// get the distance 
double MPCController::getdistance(path_follower_msgs::TrajectoryPoint2D Traj,  Eigen::Matrix<double, NUM_STATES, 1> State){
...
void MPCController::getDesiredStates_NearestP() {
  ...
  target_waypoints_marker_array.markers.clear();
...
// recycle to get the desired trajectory nodes group
  for (int i = 0; i < p_horizon + 1; i++) {
    // if the trajectory ends, stop mpc
    if(i==0 && ( traj_.point[(int)traj_.point.size()-1].t - traj_.point[node].t) < (t - start_time))
      mpc_flag = false;
  ...
  //if(plan_flag < 20 && mpc_pub_flag >= 1){
    ...
    p.x = des_traj_.x;
    p.y = des_traj_.y;    
    target_waypoints_marker_array.markers.push_back(displayWayPoint(p));
  //plan_flag ++;
  //}
  }
  publishTargetWaypointsMarkers();
...
//  Eigen::MatrixXd Qbar_Su(NUM_EXT_STATES * (p_horizon+1),  NUM_CONTROLS * p_horizon);
  ...
  Eigen::Matrix<double, p_horizon-1, NUM_CONTROLS * p_horizon> Au;
  Eigen::Matrix<double, p_horizon-1,1> lb_A, ub_A;
  Eigen::Matrix<double, 1, NUM_CONTROLS*2> Au_unit;
  Eigen::Matrix<double, 1, 1> lb_A_unit, ub_A_unit;
  Eigen::Matrix<double, NUM_CONTROLS * p_horizon, 1> uOpt_matrix;
...
// Set QProblem
  QProblem example( NUM_CONTROLS * p_horizon,p_horizon-1);
  Options options;
...
// constraint on U
  ...
  lb_unit <<  -20.0,                                    // u_dot: m/s^2
              -p_vs.param.max_wheel_angle;    // wheel_angle: rad
  ub0_unit << 20.0,
              min(p_vs.param.max_wheel_angle, steering_current_/p_vs.param.steering_ratio+2.5/p_hertz);
  lb0_unit << -20.0,
              max(-p_vs.param.max_wheel_angle, steering_current_/p_vs.param.steering_ratio-2.5/p_hertz);
  cout<<min(p_vs.param.max_wheel_angle, steering_current_+2.5/p_hertz)<<" "<<max(-p_vs.param.max_wheel_angle, steering_current_-2.5/p_hertz)<<endl;          
  ub_A_unit << 2.5/p_hertz;
  lb_A_unit << -2.5/p_hertz;
...
/* Get and print solution of first QP. */ 
  ...
  for (int i = 0; i < p_horizon; i++) {
    ...
    s_next_.vel_east = s_next(3,0);
    s_next_.vel_north  = s_next(4,0);
  ...
  //  cout<<"u_out="<<u_out->col(i)<<endl;
    ...
    if(i == 0){
      ...
      s_close_.vel_east = s_next(3,0);
      s_close_.vel_north  = s_next(4,0);
...
void MPCController::mpc_2( const Eigen::Matrix<double, NUM_STATES, 1> &s0, 
                            const Eigen::Matrix<double, NUM_CONTROLS, Dynamic> &u0,
                            const Eigen::Matrix<double, NUM_STATES, Dynamic> &s_star, 
                            const Eigen::Matrix<double, NUM_CONTROLS, 1> &u_prev1, 
                            const Eigen::Matrix<double, NUM_CONTROLS, 1> &u_prev2,
  ...
  Eigen::Matrix<double, NUM_STATES, NUM_STATES> A[p_horizon];
  Eigen::Matrix<double, NUM_STATES, NUM_CONTROLS> B[p_horizon];
  Eigen::Matrix<double, NUM_EEXT_STATES , NUM_EEXT_STATES > P, Q, Ae[p_horizon], Qe[p_horizon];
  Eigen::Matrix<double, NUM_EEXT_STATES, NUM_CONTROLS> Be[p_horizon];
  Eigen::Matrix<double, NUM_STATES, 1> s_next;
  Eigen::Matrix<double, NUM_CONTROLS, NUM_CONTROLS> R;
  Eigen::Matrix<double, NUM_EEXT_STATES * (p_horizon+1), NUM_EEXT_STATES> Sx;
  Eigen::Matrix<double, NUM_CONTROLS * p_horizon, NUM_CONTROLS * p_horizon> Rbar, H;
  Eigen::Matrix<double, NUM_CONTROLS * p_horizon, 1> g;
  Eigen::Matrix<double, NUM_CONTROLS, 1>lb_unit, ub_unit,lb0_unit, ub0_unit;
  Eigen::Matrix<double, NUM_EEXT_STATES* (p_horizon+1), 1>  Sx_x0;
  Eigen::Matrix<double, NUM_CONTROLS * p_horizon,1> lb, ub;
  Eigen::Matrix<double, NUM_EEXT_STATES, 1> x0;
  Eigen::Matrix<double, NUM_EEXT_STATES, 1> de[p_horizon];
  Eigen::Matrix<double, NUM_EEXT_STATES * p_horizon, 1>De;
  Eigen::Matrix<double, NUM_EEXT_STATES* (p_horizon+1), 1>Xref;
  Eigen::MatrixXd Qbar(NUM_EEXT_STATES * (p_horizon+1), NUM_EEXT_STATES *(p_horizon+1));
  Eigen::MatrixXd Sd(NUM_EEXT_STATES * (p_horizon+1), NUM_EEXT_STATES*p_horizon);
  Eigen::MatrixXd Su(NUM_EEXT_STATES * (p_horizon+1), NUM_CONTROLS * p_horizon);
  Eigen::MatrixXd Sutr_Qbar(NUM_EEXT_STATES * (p_horizon+1), NUM_EEXT_STATES *(p_horizon+1));
  Eigen::Matrix<double, p_horizon-1, NUM_CONTROLS * p_horizon> Au;
  Eigen::Matrix<double, p_horizon-1,1> lb_A, ub_A;
  Eigen::Matrix<double, 1, NUM_CONTROLS*2> Au_unit;
  Eigen::Matrix<double, 1, 1> lb_A_unit, ub_A_unit;
...
//set the initial x0  
  ...
  x0.setZero();
  x0.block(0,0,NUM_STATES, 1) = s0;
  x0.block(NUM_STATES+1,0,NUM_CONTROLS,1) = u_prev1;
  x0.block(NUM_STATES+1+NUM_CONTROLS,0,NUM_CONTROLS,1) = u_prev2;
//  cout<<"x0="<<endl<<x0<<endl;
// initialize cost and dynamics matrices
  for (int i = 0; i < p_horizon; i++) {
 //   cout<<"s.col(0)"<<endl<<s.col(i)<<endl;
 //   cout<<"u0.col(i)"<<endl<<u0.col(i)<<endl;
  ...
  // initialize weight matrix
  Q.setZero();
  Q.block(0, 0, NUM_STATES+1, NUM_STATES+1) = p_Q2;
  R = Rotation2D<double> (s_star(2, p_horizon)).toRotationMatrix();
  Q.block(0, 0, 2, 2) = R.transpose() * p_Q.block(0, 0, 2, 2) * R;
//  cout<<"Qblock"<<endl<< Q.block(0, 0, NUM_STATES, NUM_STATES)<<endl;
  Q.block(NUM_STATES + 1, NUM_STATES + 1, NUM_CONTROLS, NUM_CONTROLS) = p_R_delta;
  Q.block(NUM_STATES + 1 + NUM_CONTROLS, NUM_STATES + 1, NUM_CONTROLS, NUM_CONTROLS) = -p_R_delta;
  Q.block(NUM_STATES + 1, NUM_STATES + 1 + NUM_CONTROLS, NUM_CONTROLS, NUM_CONTROLS) = -p_R_delta;
  Q.block(NUM_STATES + 1 + NUM_CONTROLS, NUM_STATES + 1 + NUM_CONTROLS, NUM_CONTROLS, NUM_CONTROLS) = p_R_delta;
  P = Q;
  ...
  // initialize extended dynamics matrices
    Ae[i].setZero();
    Ae[i].block(NUM_STATES + 1 + NUM_CONTROLS, NUM_STATES + 1, NUM_CONTROLS, NUM_CONTROLS) = Eigen::Matrix<double, NUM_CONTROLS, NUM_CONTROLS>::Identity();
    Be[i].setZero();
    Be[i].block(NUM_STATES + 1, 0, NUM_CONTROLS, NUM_CONTROLS) = Eigen::Matrix<double, NUM_CONTROLS, NUM_CONTROLS>::Identity();
    Ae[i].block(0, 0, NUM_STATES, NUM_STATES) = A[i];
    Ae[i].block(NUM_STATES, 0, 1, NUM_STATES) = A[i].block(5,0,1,NUM_STATES);
    Ae[i](NUM_STATES,NUM_STATES-1) = Ae[i](NUM_STATES,NUM_STATES-1) - 1;
    Be[i].block(0, 0, NUM_STATES, NUM_CONTROLS) = B[i];
  }
...
// Define Sx
  Sx.block(0, 0, NUM_EEXT_STATES, NUM_EEXT_STATES) = Eigen::Matrix<double, NUM_EEXT_STATES, NUM_EEXT_STATES>::Identity(); 
  for(int i=0; i<p_horizon; i++){
    Sx.block(NUM_EEXT_STATES * (i+1), 0, NUM_EEXT_STATES, NUM_EEXT_STATES) = Ae[i] * Sx.block(NUM_EEXT_STATES * i, 0, NUM_EEXT_STATES, NUM_EEXT_STATES);
  }
  //cout<<"Sx="<<endl<<Sx<<endl;

// Define Su, Sd
  for (int column =0; column < p_horizon; column++){
    Su.block(NUM_EEXT_STATES *(column+1), NUM_CONTROLS * column, NUM_EEXT_STATES, NUM_CONTROLS) = Be[column];
    Sd.block(NUM_EEXT_STATES*(column+1), NUM_EEXT_STATES*column, NUM_EEXT_STATES,NUM_EEXT_STATES) = Eigen::Matrix<double, NUM_EEXT_STATES, NUM_EEXT_STATES>::Identity();
    for (int row = column +1; row < p_horizon; row++){
      Su.block(NUM_EEXT_STATES *(row+1), NUM_CONTROLS * column, NUM_EEXT_STATES, NUM_CONTROLS) = Ae[row-1] * Su.block(NUM_EEXT_STATES * row, NUM_CONTROLS* column, NUM_EEXT_STATES, NUM_CONTROLS);    
      Sd.block(NUM_EEXT_STATES * (row+1), NUM_EEXT_STATES* column, NUM_EEXT_STATES, NUM_EEXT_STATES) = Ae[row-1] * Sd.block(NUM_EEXT_STATES * row, NUM_EEXT_STATES* column, NUM_EEXT_STATES, NUM_EEXT_STATES);
    }
  }
 //  cout<<"Su="<<endl;
  ...
  //  Sx.block(NUM_EEXT_STATES * (i+1), 0, NUM_EEXT_STATES, NUM_EEXT_STATES) = Ae[i] * Sx.block(NUM_EEXT_STATES * i, 0, NUM_EEXT_STATES, NUM_EEXT_STATES);
    de[i].setZero();
    de[i].block(0,0,NUM_STATES,1) =  s.col(i+1) - A[i] * s.col(i) -B[i] * u0.col(i);
    De.block(NUM_EEXT_STATES*i, 0,NUM_EEXT_STATES,1) = de[i];
  }
  ...
  Xref.setZero();
  for(int i=0;i<(p_horizon+1);i++){
      Xref.block(NUM_EEXT_STATES*i,0,NUM_STATES,1) = s_star.col(i);
  }
  ...
  for (int i=0; i< (p_horizon+1); i++){ 
    if (i< p_horizon){
      Rbar.block(NUM_CONTROLS * i, NUM_CONTROLS * i, NUM_CONTROLS, NUM_CONTROLS) = p_R;
      Qbar.block(NUM_EEXT_STATES * i, NUM_EEXT_STATES * i, NUM_EEXT_STATES, NUM_EEXT_STATES) = Qe[i];
      /*Xref.block(NUM_EEXT_STATES*i,0,NUM_STATES,1) = s_star.col(i);
      Sx.block(NUM_EEXT_STATES * (i+1), 0, NUM_EEXT_STATES, NUM_EEXT_STATES) = Ae[i] * Sx.block(NUM_EEXT_STATES * i, 0, NUM_EEXT_STATES, NUM_EEXT_STATES);
      de[i].setZero();
      de[i].block(0,0,NUM_STATES,1) =  s.col(i+1) - A[i] * s.col(i) -B[i] * u0.col(i);
      De.block(NUM_EEXT_STATES*i, 0,NUM_EXT_STATES,1) = de[i];*/
    }
    else {
      Qbar.block(NUM_EEXT_STATES * i, NUM_EEXT_STATES * i, NUM_EEXT_STATES, NUM_EEXT_STATES) = P;
      // Xref.block(NUM_EEXT_STATES * i,0,NUM_STATES,1) = s_star.col(i);
    }
  }
...
// constraint on U
  ...
  cout<<min(p_vs.param.max_wheel_angle, steering_current_+0.5/p_hertz)<<" "<<max(-p_vs.param.max_wheel_angle, steering_current_-0.5/p_hertz)<<endl;          
