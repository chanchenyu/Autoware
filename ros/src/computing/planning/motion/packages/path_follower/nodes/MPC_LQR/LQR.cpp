...
MPCController::MPCController() : nh_(), received_applanix_state_(false),
                   received_trajectory_(false), flag_(true), flag(0), s_flag(0),mpc_flag(true), plan_flag(0), next_flag(0), mpc_pub_flag(0),
...
steering_pub_ = nh_.advertise<path_follower::SteeringCmd>("vehicle/steering_cmd",1);
...
s_close_pub_ = nh_.advertise<path_follower_msgs::ApplanixPose>("s_close",1);

traj_plan_pub_ =nh_.advertise<path_follower_msgs::traj_plan>("des_traj_",30);
//applanix_pub_ = nh_.advertise<path_follower::ApplanixPose>("applanix_",1);
}
...
void MPCController::currentstateHandler(const path_follower_msgs::state_Dynamic current_state){
  ...
  if (flag_ && received_applanix_state_){
...
//    }
  ...
  //    des_states_(5, i) =  (1 - alpha) * p1.kappa + alpha * p2.kappa;
    ...
    traj_plan_.x=des_states_(0,i);
    traj_plan_.y=des_states_(1,i);
    traj_plan_.theta=des_states_(2,i);
    traj_plan_.v=des_states_(3,i);
    traj_plan_.rate_yaw=des_states_(5,i);

    if(plan_flag < 20 && mpc_pub_flag >= 1){
          traj_plan_pub_.publish(traj_plan_); 
          plan_flag ++;
    }
...
//}
  ...
  for (int i = 0; i < p_horizon; i++) {
    ...
    if(next_flag<20 && mpc_pub_flag >= 1){
